...
`launchRegion` in `frontend/src/lib/env.ts` gates quote search, the suburb
autocomplete, and quote creation/edit to suburbs whose `suburb.launch_region`
column matches. Enforced both server-side (`find_published_quotes`) and in
the suburb query, so it can't be bypassed via a crafted request. It's
hardcoded to `canberra` for all environments. To add a new region, backfill
`launch_region` for its suburbs (see the `canberra` example in
`supabase/seed.sql`) and update `launchRegion` to match.
